...
function searchCenters() {
  ...
  centers.forEach(center => {
      resultsDiv.innerHTML += `<div><h3>${center.city}</h3></div><br><div><p>${center.address}</p><p>Type: ${center.type}</p><p>Contacts: ${center.contacts.map(contact => `${contact.name} - ${contact.phone}`).join(", ")}</p></div>`;
